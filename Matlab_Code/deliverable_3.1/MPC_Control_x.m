classdef MPC_Control_x < MPC_Control
  ...
  methods
    ...
    function ctrl_opt = setup_controller(mpc)
      ...
      figure()
      subplot(1,3,1) 
      terminal_set.projection(1:2).plot('alpha', 0.3)
      title('Terminal set projection on dim 1:2')
      xlabel('Pitch angular velocity in [rad/s]')
      ylabel('Pitch angle in [rad]')
      
      subplot(1,3,2) 
      terminal_set.projection(2:3).plot('alpha', 0.3)
      title('Terminal set projection on dim 2:3')
      xlabel('Pitch angle in [rad]')
      ylabel('linear velocity along x in [m/s]')
      
      subplot(1,3,3) 
      terminal_set.projection(3:4).plot('alpha', 0.3)
      title('Terminal set projection on dim 3:4')
      xlabel('linear velocity along x in [m/s]')
      ylabel('position along x in [m]')
      
      sgt = sgtitle('Projections of terminal set for system x','Color','black');
      sgt.FontSize = 20;
